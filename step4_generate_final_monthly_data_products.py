...
tmmx_csvs = [
    ...
    if f.endswith("tmmx.csv")
]
tmmx_csvs.sort()
for i in tqdm(range(len(tmmx_csvs))):
    df_tmmx = pd.read_csv(
        os.path.join(daily_measure_csv_dir, tmmx_csvs[i]), index_col=0, parse_dates=[0]
    )
    df_tmmn = pd.read_csv(
        os.path.join(daily_measure_csv_dir, tmmx_csvs[i].replace("tmmx", "tmmn")),
        index_col=0,
        parse_dates=[0],
    )
...
pr_csvs = [
    os.path.join(daily_measure_csv_dir, f)
    for f in os.listdir(daily_measure_csv_dir)
    if f.endswith("pr.csv")
]
pr_csvs.sort()
for i in tqdm(range(len(pr_csvs))):
    year = os.path.basename(pr_csvs[i]).split("_")[0]
    df_pr = pd.read_csv(
        os.path.join(daily_measure_csv_dir, pr_csvs[i]), index_col=0, parse_dates=[0]
    )
    df_pr_bool = df_pr > 0
    ...
    df_final = pd.concat(
        [
            ...
            df_total_pr_monthly,
            df_avg_pr_monthly,
            df_avg_pr_only_pr_days_monthly,
        ],
        axis=1,
    )
...
hi_csvs = [
    os.path.join(daily_measure_csv_dir, f)
    for f in os.listdir(daily_measure_csv_dir)
    if f.endswith("heat_index.csv")
]
hi_csvs.sort()
for i in tqdm(range(len(hi_csvs))):
    year = os.path.basename(hi_csvs[i]).split("_")[0]
    df_hi = pd.read_csv(
        os.path.join(daily_measure_csv_dir, hi_csvs[i]), index_col=0, parse_dates=[0]
    )
    ...
    df_final = pd.concat(
        [
            df_hi_level0_num_monthly,
            df_hi_level0_pct_monthly,
            df_hi_level1_num_monthly,
            df_hi_level1_pct_monthly,
            df_hi_level2_num_monthly,
            df_hi_level2_pct_monthly,
            df_hi_level3_num_monthly,
            df_hi_level3_pct_monthly,
            df_hi_level4_num_monthly,
            df_hi_level4_pct_monthly,
        ],
        axis=1,
    )
